...
// Method to handle page redirection logic
  void pageRedirect() async {
    final user = _auth.currentUser;

    if (user != null) {
      if (user.emailVerified) {
        // Get the user's role and existence status from Firestore
        ...
        if (userDoc.exists) {
          ...
          if (role == 'customer') {
            if (existing) {
              ...
              // Redirect to customer setup profile
              Get.offAllNamed('/customer/setup_profile');
            }
          } else {
            // Log out if the role is invalid
            await logOut();
            ToastNotif(message: 'Access Denied', title: 'Error')
                .showErrorNotif(Get.context!);
          }
        } else {
          // Log out if the user document does not exist
          await logOut();
          ToastNotif(message: 'User is not found.', title: 'Error')
              .showErrorNotif(Get.context!);
        }
      } else {
      ...
      // If user is not authenticated, check first-time status in local storage
